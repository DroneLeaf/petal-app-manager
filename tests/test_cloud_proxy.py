...
from petal_app_manager.proxies.cloud import CloudDBProxy, Config
...
async def proxy() -> AsyncGenerator[CloudDBProxy, None]:
    ...
    with patch.object(Config, 'ACCESS_TOKEN_URL', 'https://example.com/token'), \
         patch.object(Config, 'CLOUD_ENDPOINT', 'https://api.example.com'):
...
async def test_get_item_soft_deleted(proxy: CloudDBProxy):
    ...
    mock_response = {"data": {"id": "123", "name": "Test Item", "deleted": True}, "success": True}
...
async def test_configuration_validation():
    ...
    with patch.object(Config, 'ACCESS_TOKEN_URL', ''), \
         patch.object(Config, 'CLOUD_ENDPOINT', 'https://api.example.com'):
    ...
    with patch.object(Config, 'ACCESS_TOKEN_URL', 'https://example.com/token'), \
         patch.object(Config, 'CLOUD_ENDPOINT', ''):
